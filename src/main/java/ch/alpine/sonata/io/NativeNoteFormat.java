...
public enum NativeNoteFormat implements NoteFormat {
  INSTANCE;

  public static final int CENTER_OCTAVE = 5;
  ...
  public String format(Note note) {
    ...
    Diatone diatone = diatoneAlter.diatone();
    Alter alter = diatoneAlter.alter();
    int octave = tone.ivory().octave();
    int ticks = note.ticks();
    return diatone.lowerCase() + alter.getAccidental() + (octave - CENTER_OCTAVE) + " " + ticks;
  }
